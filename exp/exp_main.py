...
class Exp_Main(Exp_Basic):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for index in range(self.num_sources):
                ...
                for batch_source, batch_target in zip(train_loader, target_loader):
                    ...
                    if (i + 1) % 50 == 0:
                        print("\titers: {0}, epoch: {1} | TST Loss: {2:.7f} | CORAL Loss: {3:.7f}".format(i + 1, epoch + 1, loss.item(), domain_loss.item()))
    ...
    def test(self, setting, test=0):
        ...
        if test:
            print('loading model')
            self.model.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, 'checkpoint_coral_traf_weat_no_revin.pth'), map_location=self.device))
